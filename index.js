...
function createClient(label) {
  const client = new Client({
    ...
    sweepers: { messages: { interval: 60, lifetime: 0 } },
...
const clients = {
  bot_1: createClient('bot_1'),
  bot_2: createClient('bot_2')
};

function loadCommands(client, dir, context = []) {
  const files = fs.readdirSync(dir, { withFileTypes: true });
  for (const file of files) {
    if (file.isDirectory()) {
      loadCommands(client, path.join(dir, file.name), [...context, file.name]);
    } else if (file.name.endsWith('.js') && !file.name.startsWith('archived')) {
      const command = require(path.join(dir, file.name));
      const commandName = command.data?.name || file.name.replace('.js', '');
      const targets = command.targets || ['bot_1', 'bot_2'];
...
for (const [label, client] of Object.entries(clients)) {
  ...
  rest.put(Routes.applicationCommands(clientIds[label]), { body: slashCommands })
